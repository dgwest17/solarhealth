...
/**
 * Typical OUT-OF-WARRANTY replacement costs (parts + labor), California
 * residential. EDIT THESE to your real service pricing — they drive the
 * report's "after warranty ends" cost exposure and the service-plan upsell.
 * Figures are planning estimates, marked as such on the report.
 */
export const REPLACEMENT_COSTS = {
  'Solar panels': { parts: 450, labor: 400, note: 'per-panel incident (glass/hotspot), not full-array' },
  'Inverter': { parts: 2400, labor: 900, note: 'string inverter swap; microinverters fail per-unit (~$350 ea installed)' },
  'Battery': { parts: 9500, labor: 1600, note: 'full pack replacement at end of life' }
};

export function replacementCostFor(kind) {
  return REPLACEMENT_COSTS[kind] || null;
}

/**
 * Sum the out-of-warranty exposure landing within the next `horizonYears`:
 * items whose expected service year falls in [now, now+horizon] and whose
 * warranty has ended by then.
 */
export function projectedEquipmentExposure(items, horizonYears = 10) {
  const nowYear = new Date().getFullYear();
  const out = [];
  let total = 0;
  for (const it of items) {
    const rc = replacementCostFor(it.kind);
    if (!rc) continue;
    const inWindow = it.serviceYear >= nowYear && it.serviceYear <= nowYear + horizonYears;
    const outOfWarranty = it.serviceYear >= it.warrantyEnds;
    if (inWindow && outOfWarranty) {
      const cost = rc.parts + rc.labor;
      out.push({ kind: it.kind, year: it.serviceYear, parts: rc.parts, labor: rc.labor, cost, note: rc.note });
      total += cost;
    }
  }
  return { items: out, total, horizonYears };
}
