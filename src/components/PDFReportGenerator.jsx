 ...
 */
const PDFReportGenerator = ({ inputs, calculations, narrative, branding }) => {
  const [clientName, setClientName] = useState('');
  const [preparedBy, setPreparedBy] = useState('');

  // White-label branding (Phase 3): agency settings override the defaults
  const brandColor = branding?.primary_color || '#d4a017';
  const brandName = preparedBy || branding?.company_name || '';
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  header { border-bottom: 3px solid ${brandColor}; padding-bottom: 14px; margin-bottom: 24px;
           display: flex; justify-content: space-between; align-items: flex-end; }
  .brand { font-family: Helvetica, Arial, sans-serif; font-size: 20pt; font-weight: bold; color: #14213d; }
  .brand span { color: ${brandColor}; }
  ...
  h2 { font-family: Helvetica, Arial, sans-serif; font-size: 13pt; color: #14213d;
       border-left: 4px solid ${brandColor}; padding-left: 10px; margin: 22px 0 12px; }
  h4 { font-family: Helvetica, Arial, sans-serif; font-size: 9pt; text-transform: uppercase;
       letter-spacing: 1px; color: ${brandColor}; margin-bottom: 5px; }
  p { margin-bottom: 6px; }
  ...
  .headline { font-size: 14pt; font-style: italic; color: #14213d; margin: 14px 0 18px;
              padding: 12px 16px; background: #faf6ec; border-left: 4px solid ${brandColor}; }

  .kpis { display: flex; gap: 12px; margin: 14px 0; }
  .kpi { flex: 1; border: 1px solid ${brandColor}; border-radius: 6px; padding: 12px; text-align: center; }
  ...
  .gold { color: ${brandColor}; font-weight: bold; }

  .action { display: flex; gap: 10px; margin-bottom: 10px; align-items: flex-start; }
  .action-num { width: 22px; height: 22px; border-radius: 50%; background: ${brandColor}; color: #fff;
  ...
  .cta h3 { font-family: Helvetica, Arial, sans-serif; color: ${brandColor}; font-size: 12pt; margin-bottom: 6px; }
...
<div class="page">
  <header>
    <div style="display:flex; align-items:center; gap:12px;">
      ${brandLogo ? `<img src="${brandLogo}" style="height:42px; object-fit:contain;" />` : ''}
      <div class="brand">${brandName ? brandName : 'Solar Financial <span>Audit</span>'}</div>
    </div>
    <div class="meta">
      ${clientName ? `Prepared for: <strong>${clientName}</strong><br/>` : ''}
      ${brandName ? `Prepared by: ${brandName}<br/>` : ''}
      ${today}
    </div>
  </header>
  ...
  <footer>${brandName ? brandName : 'Solar Financial Audit'} • Rates sourced from CPUC reports • This report is an estimate based on data provided.</footer>
</div>
...
<div class="page">
  ...
  <footer>${brandName ? brandName : 'Solar Financial Audit'} • Questions? Contact your solar advisor to review these results.</footer>
</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
        ...
        <div>
          ...
          <input
            ...
            placeholder={branding?.company_name || 'e.g. SunCoast Solar'}
            className="w-full px-3 py-2 border border-amber-400/30 rounded-lg bg-slate-900/60 text-slate-200"
          />
        </div>
      </div>
